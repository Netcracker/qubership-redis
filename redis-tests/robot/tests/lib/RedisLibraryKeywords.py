...
class RedisLibraryKeywords(object):
    ...
    def connect_to_redis(self, redis_host, redis_port=6379, db=0, redis_password=None, tls_enabled=False, ca_path=""): # pragma: no cover
        ...
        try:
            if tls_enabled:
                logger.info(f'tls is {tls_enabled}, ca path is {ca_path}')
                redis_conn = redis.StrictRedis(host=redis_host, port=redis_port, db=db,  password=redis_password, ssl=True, ssl_ca_certs=ca_path, ssl_check_hostname=False,
